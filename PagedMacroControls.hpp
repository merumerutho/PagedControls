#pragma once
#ifndef PAGED_MACRO_CONTROLS_H
#define PAGED_MACRO_CONTROLS_H

#include <cstddef>
#include <cstdint>
#include <cmath>

// PagedMacroControls - "parameter pages + soft-takeover pickup + LED indicator"
// (the original PagedControls) with a folded-in *virtualization (macro) layer*:
...
//                       Default = GridSize -> no derived nodes -> behaves exactly
//                       like the original PagedControls.
//     NumCurves (>=1)   number of reshaping curves (default 1 = linear).
//     CurveRes  (>=2)   samples per curve LUT (default 33).
//
// A back-compat alias `pagedctl::PagedControls<P,K>` is provided at the bottom so
...
public:
    ...
    // --- Legacy Init (no routing): matches the original PagedControls signature so
    // the `PagedControls` alias and existing call sites compile unchanged. ---
    void Init(const float (&defaults)[NumPages][NumKnobs],
              float     pickup_threshold = 0.03f,
              LedTiming led = {15, 40, 110})
    {
    ...
    // Call once per block (the UI tick: paging + soft-takeover + LED). `knobs` is
    // NumKnobs values in 0..1; `advance_page` is the (already-debounced) button-tap
    // edge. On a page change the pickup is reset so knobs must be re-caught.
    //
    ...
    //     loop) so the (possibly costly) macro eval stays OUT of the audio ISR while
    //     paging/pickup/LED keep their required block-rate cadence here.
...
// Back-compat alias: the original name, no routing (NumNodes == GridSize).
template <size_t NumPages, size_t NumKnobs = 4>
using PagedControls = PagedMacroControls<NumPages, NumKnobs>;

} // namespace pagedctl

#endif // PAGED_MACRO_CONTROLS_H
